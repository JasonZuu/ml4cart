...
class CrossAttnFusionModel(nn.Module):
    ...
    def _build_tokens(
        ...
        b = x_seq.size(0)
        query_tokens = [seq_token, self.track_norm(self.track_encoder(x_track))]
        ...
        kv_tokens = [
            self.tumor_norm(self.tumor_encoder(tumor_input)),
            self.stroma_norm(self.stroma_encoder(x_stroma)),
            self.immune_norm(self.immune_encoder(x_immune)),
        ]
        return query_tokens, kv_tokens

    def _fuse_tokens(
        self,
        query_tokens_stacked: torch.Tensor,
        kv_tokens_stacked: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        attn_out, attn_weights = self.cross_attn(
            query=query_tokens_stacked,
            key=kv_tokens_stacked,
            value=kv_tokens_stacked,
            need_weights=True,
        )
        tokens = self.post_attn_norm(query_tokens_stacked + self.attn_dropout(attn_out))
        ffn_out = self.ffn(tokens)
        tokens = self.post_ffn_norm(tokens + self.ffn_dropout(ffn_out))
        # gates = self.query_gate(tokens)
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_tokens, kv_tokens = self._build_tokens(x_seq, x_track, x_pdosize, x_antigen, x_stroma, x_immune)
        query_tokens_stacked = torch.stack(query_tokens, dim=1)
        kv_tokens_stacked = torch.stack(kv_tokens, dim=1)
        fused_tokens, _ = self._fuse_tokens(query_tokens_stacked, kv_tokens_stacked)
    ...
    def get_attn_weights(
        ...
        query_tokens_stacked = torch.stack(query_tokens, dim=1)
        kv_tokens_stacked = torch.stack(kv_tokens, dim=1)
        _, attn_weights = self._fuse_tokens(query_tokens_stacked, kv_tokens_stacked)
        return attn_weights
