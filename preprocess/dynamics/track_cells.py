...
from dynamics_model.config import DATA_DIR, DATA_DIRS, FIJI_PATH, JAVA_ARGUMENTS, CELL_TRACKING_DATASET_CONFIGS
import pandas as pd

def track_with_trackmate(images_folder, subcase_names, case_name, fiji_path, prefix = "", specific_thresholds ={}, 
                         include_spots_without_track_id = False, ignore_duplicate_warning = False, java_arguments = "", output_root = None):

    # Check if output folder can be created
    output_base = output_root or DATA_DIR
    output_folder_path = os.path.join(output_base, case_name)
    ...
    # Print app info defensively across ImageJ gateway versions without static attribute access
    try:
        app_getter = getattr(ij, "getApp", None)
        if callable(app_getter):
            app = app_getter()
            info_getter = getattr(app, "getInfo", None)
            if callable(info_getter):
                try:
                    print(info_getter(True))
                except Exception:
                    print(str(app))
    except Exception:
        pass
    ...
    for XY_position_folder_path in sorted(os.listdir(images_folder)):
        ...
        try:
            if os.path.exists(XY_position_folder_path):
                ...
                print(f"[WARN] Path not found: {XY_position_folder_path}")
                continue
            
            # Skip processing early if outputs for this XY already exist
            subcase_folder_path = XY_position_folder_path.split(os.sep)[-2]
            XY_position = XY_position_folder_path.split(os.sep)[-1]
            matched_subcase = None
            for subcase_name in subcase_names:
                if subcase_name in subcase_folder_path:
                    matched_subcase = subcase_name
                    break
            if matched_subcase is not None:
                spots_output_path = os.path.join(output_folder_path, f"{prefix}{matched_subcase}_{XY_position}_spots.csv")
                track_output_path = os.path.join(output_folder_path, f"{prefix}{matched_subcase}_{XY_position}_tracks.csv")
                if os.path.exists(spots_output_path) and os.path.exists(track_output_path):
                    print(f"Outputs exist; skipping: {spots_output_path} and {track_output_path}")
                    continue
        ...
        # -------------------------------------------------------------
            ...
            # Construct a plain Python list of floats; JPype will auto-convert to double[]
            qualities_list = [float(q) for q in qualities]

            initial_quality_threshold = float(TMUtils.otsuThreshold(qualities_list))
            print("Initial threshold:", initial_quality_threshold)
            settings.initialSpotFilterValue = Double(initial_quality_threshold)
        ...
        # -------------------------------------------------------------------------------
            ...
            for subcase_name in subcase_names:
                if subcase_name in subcase_folder_path:
                    ...
                    if os.path.exists(spots_output_path):
                        print(f"Exists, skipping write: {spots_output_path}")
                    else:
                        df_spots.to_csv(spots_output_path, index=False)
                    if os.path.exists(track_output_path):
                        print(f"Exists, skipping write: {track_output_path}")
                    else:
                        df_tracks.to_csv(track_output_path, index=False)
                    print("Completed: saved or skipped existing outputs for this XY.")
                    print(f"Spots: {spots_output_path}")
                    print(f"Tracks: {track_output_path}\n")
                    break
            else:
            ...
            # Explicit cleanup to prevent JVM/ImageJ OOM across iterations
            try:
                if imp is not None:
                    try:
                        imp.flush()
                ...
                print(f"[WARN] Failed to close image: {e}")
            # Close any lingering ImageJ windows and free memory
            try:
                IJ.run("Close All")
            except Exception:
                pass
            ...
            try:
                ...
                pass
            # Drop references so JPype can release Java objects
            trackmate = None
            model = None
            settings = None
            imp = None
...
def _resolve_path(path, data_roots):
    if path is None:
        return path, None
...
def run_tracking_from_config(data_roots = None, ignore_duplicate_warning = True, include_spots_without_track_id = False):
    data_roots = _normalize_roots(data_roots)
    for case in CELL_TRACKING_DATASET_CONFIGS:
        cfg = CELL_TRACKING_DATASET_CONFIGS[case]
        images_folder, root = _resolve_path(cfg["images_folder"], data_roots)
        output_root = root or DATA_DIR
        ...
        for subfolder in sorted(os.listdir(images_folder)):
            ...
            track_with_trackmate(images_subfolder, 
                                ...
                                FIJI_PATH, 
                                prefix = cfg["prefix"],
                                specific_thresholds = cfg.get("specific_thresholds", {}),
                                java_arguments = JAVA_ARGUMENTS,
                                include_spots_without_track_id = include_spots_without_track_id, 
